...
{
    public abstract class JahomProjectTestBase : AbpIntegratedTestBase<JahomProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected JahomProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<JahomProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<JahomProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<JahomProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<JahomProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<JahomProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JahomProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<JahomProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JahomProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<JahomProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<JahomProjectDbContext, Task<T>> func)
        {
            T result;
